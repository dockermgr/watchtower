...
# - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
##@Version           :  202309102314-git
# @@Author           :  Jason Hempstead
# @@Contact          :  jason@casjaysdev.pro
# @@License          :  LICENSE.md
# @@ReadME           :  install.sh --help
# @@Copyright        :  Copyright: (c) 2023 Jason Hempstead, Casjays Developments
# @@Created          :  Sunday, Sep 10, 2023 23:14 EDT
# @@File             :  install.sh
...
REPO="${DOCKERMGRREPO:-https://github.com/$SCRIPTS_PREFIX}/watchtower"
APPVERSION="$(__appversion "$REPO/raw/$REPO_BRANCH/version.txt")"
# - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
# Defaults variables
APPNAME="watchtower"
export INSTDIR="$HOME/.local/share/CasjaysDev/$SCRIPTS_PREFIX/watchtower"
export DOCKERMGR_CONFIG_DIR="${DOCKERMGR_CONFIG_DIR:-$HOME/.config/myscripts/$SCRIPTS_PREFIX}"
# - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
# Set the mountpoint directory
export APPDIR="$HOME/.local/share/srv/docker/watchtower"
...
# - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
__show_post_message() {
  printf_blue 'To use api: curl -q -LSsf -H "Authorization: Bearer '$CONTAINER_USER_ADMIN_PASS_HASH'" '$NGINX_PROXY_URL/v1/update''
  return 0
}
# - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
...
HUB_IMAGE_URL="casjaysdevdocker/watchtower"
# - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
...
# - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
# Set container hostname and domain - Default: [watchtower.$SET_HOST_FULL_NAME] [$SET_HOST_FULL_DOMAIN]
CONTAINER_HOSTNAME=""
...
HOST_CRON_COMMAND='curl -q -LSsf -H \"Authorization: Bearer $CONTAINER_USER_ADMIN_PASS_HASH\" \"$NGINX_PROXY_URL/v1/update\"'
# - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
...
WATCHTOWER_HTTP_API_TOKEN="$CONTAINER_USER_ADMIN_PASS_HASH"
EOF
}
# - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
# Set custom script - saves to /config/env/watchtower.script.sh
...
CONTAINER_HOSTNAME="${CONTAINER_HOSTNAME:-${APPNAME:-watchtower}}"
...
if [ -n "$WATCHTOWER_USERNAME" ]; then
  CONTAINER_USER_NAME="$WATCHTOWER_USERNAME"
fi
if [ -n "$CONTAINER_USER_NAME" ]; then
  CONTAINER_USER_NAME="${WATCHTOWER_USERNAME:-${CONTAINER_USER_NAME:-$DEFAULT_USERNAME}}"
fi
if [ -n "$CONTAINER_USER_NAME" ]; then
...
if [ -n "$WATCHTOWER_PASSWORD" ]; then
  CONTAINER_USER_PASS="$WATCHTOWER_PASSWORD"
fi
if [ -n "$CONTAINER_USER_PASS" ]; then
  if [ "$CONTAINER_USER_PASS" = "random" ]; then
    CONTAINER_USER_PASS="$(__password "${CONTAINER_PASS_LENGTH:-16}")"
  fi
  CONTAINER_USER_PASS="${WATCHTOWER_PASSWORD:-${CONTAINER_USER_PASS:-$DEFAULT_PASSWORD}}"
fi
if [ -n "$CONTAINER_USER_PASS" ]; then
