...
# - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
##@Version           :  202409241301-git
# @@Author           :  Jason Hempstead
# @@Contact          :  jason@casjaysdev.pro
# @@License          :  LICENSE.md
# @@ReadME           :  install.sh --help
# @@Copyright        :  Copyright: (c) 2024 Jason Hempstead, Casjays Developments
# @@Created          :  Tuesday, Sep 24, 2024 13:01 EDT
# @@File             :  install.sh
...
HOST_CRON_COMMAND='curl -q -LSsf -H \"Authorization: Bearer ${CONTAINER_API_KEY_TOKEN}\" \"$NGINX_PROXY_URL/v1/update\" >/dev/null 2>&1'
# - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
...
if [ "$NINGX_VHOSTS_WRITABLE" = "true" ]; then
  ...
  if [ "$HOST_NGINX_UPDATE_CONF" = "yes" ] && [ -f "$INSTDIR/nginx/proxy.conf" ]; then
    ...
    if grep -ERs "^proxy_intercept_errors| proxy_intercept_errors" "$NGINX_DIR/global.d" | grep -vq '#'; then
      sed -i "s|proxy_intercept_errors|proxy_intercept_errors|g" "$NGINX_VHOSTS_CONF_FILE_TMP" &>/dev/null
    fi
